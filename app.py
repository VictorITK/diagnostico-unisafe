...
# Inteligência baseada no Guia NR-01
MAPA_RISCOS = {
...
def index():
    ...
    try:
        ...
        modo = dados.get('modo', 'individual').lower()
    except:
        setores, modo = ["Geral"], "individual"

    # Se for individual e já tiver participado, bloqueia
    if modo == "individual" and request.cookies.get(f'participou_{cliente_id}'):
        return "<h1>Atenção</h1><p>Você já participou desta pesquisa. O anonimato e a unicidade são garantidos pela UNISAFE.</p>"

    return render_template('index.html', questoes=QUESTOES, opcoes=OPCOES, setores=setores, cliente=cliente_id.title(), modo=modo)
